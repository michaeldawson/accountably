...
RSpec.describe Bank::Adapter::Selenium::NAB::Account do
  ...
  describe '#reconcile' do
    context 'when the session returns session rows' do
      let(:transaction_parser) { double(:transaction) }
      let(:stubbed_transaction_row_element) { double(:capybara_node) }

      before do
        allow(Bank::Adapter::Selenium::NAB::Account::Transaction).to receive(:new).and_return(transaction_parser)
      ...
      it 'requests that all transactions be parsed' do
